...
class QueryManager {
  ...
  parseGroupByReply(opts, reply) {
    return __awaiter(this, void 0, void 0, function* () {
      const deserialize = yield Deserialize.use(this.check.dbVersionSupport);
      return Serialize.search.isGroupBy(opts) ? deserialize.queryGroupBy(reply) : deserialize.query(reply);
    });
  }
    ...
    return this.check
      ...
      .then(({ search, supportsTargets, supportsWeightsForTargets }) => {
        ...
        switch (type) {
          case 'audio':
            send = (media) =>
              search.withNearAudio(Serialize.search.nearAudio(Object.assign({ audio: media }, args), opts));
            break;
          case 'depth':
            send = (media) =>
              search.withNearDepth(Serialize.search.nearDepth(Object.assign({ depth: media }, args), opts));
            break;
          case 'image':
            send = (media) =>
              search.withNearImage(Serialize.search.nearImage(Object.assign({ image: media }, args), opts));
            break;
          case 'imu':
            send = (media) =>
              search.withNearIMU(Serialize.search.nearIMU(Object.assign({ imu: media }, args), opts));
            break;
          case 'thermal':
            send = (media) =>
          ...
          case 'video':
            send = (media) =>
              search.withNearVideo(Serialize.search.nearVideo(Object.assign({ video: media }, args)));
            break;
          default:
